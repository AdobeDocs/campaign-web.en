...
1. Expand the **[!UICONTROL Extraction format]** section to access specific options related to the format of the extraction file. Available values depend on the selected format.
    ...
    * **[!UICONTROL Encoding]**: Choose the encoding of the extraction file. The default encoding is **Latin-1 (1252)**. When creating direct mail deliveries targeting recipients in CJK languages (Japanese, Korean, Simplified Chinese, Traditional Chinese), select **UTF-8 (65001)** instead. Latin-1 cannot represent CJK characters and will result in an empty file preview.
