...
When you extend the [!DNL nms:offer] schema with a custom collection link — a one-to-many relationship, for example a set of segments linked to an offer — you can expose it as an editable list directly in the offer's **[!UICONTROL Custom options]** section. Instead of managing the related records through a separate screen, the collection is rendered as a list in the offer detail, and you can create new related records inline through a dedicated dialog.
...
1. Extend the [!DNL nms:offer] schema with your custom collection, then browse to the **[!UICONTROL Schemas]** menu, open the **[!UICONTROL Marketing offers]** schema, and click **[!UICONTROL Screen edition]**.

1. In the **[!UICONTROL Detail screen configuration]** section, click the ellipsis icon above the **[!UICONTROL List of custom fields]** table and choose **[!UICONTROL Select attributes]**.
...
1. In the **[!UICONTROL Screen configuration]** tab, click **[!UICONTROL Select attributes]** and select the attributes to use when adding a new element to the list — for example a segment name and a custom field.
   ...
   ![Screenshot showing the screen configuration tab of the collection link settings dialog.](assets/offers-editable-list-2.png){zoomable="yes"}
...
1. Open an offer and expand **[!UICONTROL Custom options]**. The collection is rendered as a list with an **[!UICONTROL Add]** button.

   ![Screenshot showing the editable list rendering in the offer detail screen.](assets/offers-editable-list-3.png){zoomable="yes"}

1. Click **[!UICONTROL Add]** to open a dialog containing the attributes you configured, fill them in, and click **[!UICONTROL Confirm]**. The new element is added to the list.

   ![Screenshot showing the add element dialog for the collection.](assets/offers-editable-list-4.png){zoomable="yes"}

1. You can add multiple elements to the same list, and the offer detail can contain more than one editable list. Click **[!UICONTROL Save]** to persist the changes.

   ![Screenshot showing the editable list with multiple elements added.](assets/offers-editable-list-5.png){zoomable="yes"}
